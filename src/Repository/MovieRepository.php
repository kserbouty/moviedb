...
class MovieRepository
{
    ...
    public function getMovieCompanies(int $movie_id): array
    {
        try {
            $pdo = $this->connection->getPDO();
            $query = "SELECT id, name, logo_path, origin_country FROM movie_db.company INNER JOIN movie_db.movie_company ON company.id = movie_company.company_id WHERE movie_id = ?;";
    ...
    public function getMovieGenres(int $movie_id): array
    {
        try {
            $pdo = $this->connection->getPDO();
            $query = "SELECT id, name FROM movie_db.genre INNER JOIN movie_db.movie_genre ON genre.id = movie_genre.genre_id WHERE movie_id = ?;";
    ...
    public function getMovieLanguages(int $movie_id): array
    {
        try {
            $pdo = $this->connection->getPDO();
            $query = "SELECT iso_639_1, name FROM movie_db.language INNER JOIN movie_db.movie_language ON language.id = movie_language.language_id WHERE movie_id = ?;";
